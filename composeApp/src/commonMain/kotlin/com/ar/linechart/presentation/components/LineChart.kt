package com.ar.linechart.presentation.components
...
import com.ar.linechart.presentation.models.ValueLabel
...
fun LineChart(
    ...
    Canvas(
        modifier = modifier
            ...
            .pointerInput(drawPoints, xLabelWidth){
                detectHorizontalDragGestures { change, _ ->
                    ...
                    isShowingDataPoints =
                        (visibleDataPointsIndices.first + newSelectedIndex) in
                            visibleDataPointsIndices
                    if(isShowingDataPoints) {
                        onSelectedDataPoint(dataPoints[newSelectedIndex])
                    } else {
        ...
        // calculation of main viewport size
        ...
        val viewPortTopY = verticalPaddingPx + xLabelLineHeight + 30f
        ...
        val colorBack = linePath.copy().apply {
            lineTo(x = drawPoints.last().x, viewPortBottomY)
            lineTo(x = drawPoints.first().x, viewPortBottomY)
            lineTo(drawPoints.first().x, drawPoints.first().y)
        }
        val gradientBrush = Brush.verticalGradient(
            colors = listOf(
                Color.Green.copy(alpha = .5f),
            ...
            if (selectedDataPointIndex == index) {
                ...
                if (isTextInVisibleRange) {
                    drawText(
                        ...
                        topLeft = Offset(
                            x = textPositionX,
                            y = viewPortTopY - valueResult.size.height - 30f
                        )
                    )
                }
            }
